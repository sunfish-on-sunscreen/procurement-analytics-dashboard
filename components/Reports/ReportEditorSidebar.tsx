...
import { Settings, X, Loader2, Save, Sparkles } from "lucide-react";
import {
  type ReportConfig,
  type SectionKey,
  type DetailLevel,
...
/** The optional AI-narrative control (supplier focus only), owned by ReportEditor. */
export type NarrativeControl = {
  prompt: string;
...
/** The narrative control's status line — surfaces the MEASURED cost on success, the
 *  no-key/error fallback otherwise. */
function NarrativeStatusLine({
  status,
  result,
  canGenerate,
}: {
  ...
  if (status === "loading") {
    return <p className="text-[11px] text-muted-foreground">Generating…</p>;
  }
  if (status === "generated" && result?.available) {
    // No per-request cost on the free tier — surface the request-rate caps (what
    // actually constrains use) in place of the old dollar figure. "≈" flags that
    // Google sets these per project; see FREE_TIER_RATE_LIMITS.
    return (
      <p className="text-[11px] leading-snug text-muted-foreground">
        Generated by <span className="text-foreground">{result.model}</span> ·{" "}
        {result.usage.inputTokens} in / {result.usage.outputTokens} out · free tier ≈
        {FREE_TIER_RATE_LIMITS.requestsPerMinute}/min,{" "}
        {FREE_TIER_RATE_LIMITS.requestsPerDay.toLocaleString("en-US")}/day · inputs{" "}
        {result.inputsHash}
      </p>
    );
  }
  if (status === "unavailable") {
    const reason = result && !result.available ? result.reason : "error";
    const msg =
      reason === "no_key"
        ? "No API key configured — the brief uses the template narrative."
        : reason === "rate_limited"
          ? "Generation temporarily unavailable (free-tier rate limit reached) — the brief uses the template narrative."
          : "Generation was unavailable — the brief uses the template narrative.";
    return <p className="text-[11px] leading-snug text-muted-foreground">{msg}</p>;
  }
  if (!canGenerate) {
    return (
      <p className="text-[11px] text-muted-foreground">
        Pick a supplier and let its data load to enable generation.
      </p>
    );
  }
  return null;
...
export function ReportEditorSidebar({
  ...
  /** Present only for a supplier brief — drives the optional AI-narrative control. */
  ...
  return (
    <aside className="sticky top-0 flex h-[calc(100vh-2rem)] w-[248px] shrink-0 flex-col overflow-y-auto overflow-x-hidden border-r transition-[width] duration-150 ease-out">
      ...
      <div className="flex flex-col gap-5 p-3">
        ...
        {/* Narrative (AI draft) — supplier focus only, optional */}
        {narrative && (
          <section className="space-y-2 border-t pt-4">
            <GroupLabel>Narrative (AI draft)</GroupLabel>
            <p className="text-[11px] text-muted-foreground">
              Optional. Rewrites the brief&rsquo;s prose from the computed values —
              every number stays exactly as calculated. Edit the instruction, then
              generate.
            </p>
            <textarea
              value={narrative.prompt}
              onChange={(e) => narrative.onPromptChange(e.target.value)}
              rows={4}
              className="w-full resize-y rounded-md border bg-background px-2 py-1 text-xs leading-snug"
              placeholder="Describe how the narrative should read…"
            />
            <div className="flex items-center gap-2">
              <Button
                ...
                disabled={
                  ...
                  !narrative.prompt.trim() ||
                  narrative.status === "loading"
                }
                onClick={narrative.onGenerate}
              >
                {narrative.status === "loading" ? (
              ...
              <button
                type="button"
                onClick={narrative.onResetPrompt}
            ...
            <NarrativeStatusLine
              ...
              canGenerate={narrative.canGenerate}
            />
          </section>
        )}
      </div>
